...
export default function SimulateSensor() {
    ...
    const getButtonToShow = () => {
        ...
        console.log("Absolute energy difference <= tolerance:", Math.abs(energyDiff) <= tolerance);
        console.log("Total energy used > total expected output * 1.1:", roundedTotalEnergyUsed > roundedTotalExpectedOutput * 1.1);
            ...
            <div className=" flex mt-20 max-h-screen text-white w-full">
                <div className="border-r-2 w-1/2">
                    <div className="text-white mt-24 mb-48">
                        ...
                        <form className=" text-white ml-16 pr-20 ">
                            <div className="mb-3 w-1/2">
                                <label className="block mb-2 text-lg font-medium text-white">Longitude</label>
                                <input 
                                    type="number" 
                                    value={longitude} 
                            ...
                            <div className="mb-5 w-1/2">
                                <label className="block mb-2 text-lg font-medium text-white">Latitude</label>
                                <input 
                                    type="number" 
                                    value={latitude} 
                    ...
                    <h2 className="ml-5 mt-8 text-yellow-400 font-semibold text-2xl mb-2">Enter Voltage and Current</h2>
                            ...
                            <div className="mb-3 w-1/2">
                                <label className="block mb-2 text-lg font-medium text-white">Voltage (V)</label>
                                <input 
                                    type="number" 
                                    value={voltage} 
                            ...
                            <div className="mb-5 w-1/2">
                                <label className="block mb-2 text-lg font-medium text-white">Current (A)</label>
                                <input 
                                    type="number" 
                                    value={current} 
                        ...
                        <br />
                        {!capturing ? (
                            <button className="ml-16 text-white bg-green-600 hover:bg-green-700 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-1.5 text-center" type="button" onClick={startCapture}>
                                Start Capture
                            </button>
                        ) : (
                            <button className="ml-16 text-white bg-red-500 hover:bg-red-700 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-1.5 text-center" type="button" onClick={stopCapture}>
                                Stop Capture
                            </button>
                        )}
                    </div>
